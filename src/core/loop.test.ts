...
test("folds oldest round-trips into a compaction summary once the estimate crosses the threshold", async () => {
  ...
  const model = new MockLanguageModelV3({
    doGenerate: mockValues(toolCallResp, toolCallResp, toolCallResp, toolCallResp, toolCallResp, finalResp) as any,
  });
  ...
  const laterPrompt = JSON.stringify((model as any).doGenerateCalls.at(-1).prompt);
