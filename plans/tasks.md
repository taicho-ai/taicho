...
- [x] `prompt.ts`: root's final reply NAMES the deliverable handle and never pastes the artifact body. Assert in a Layer-1 test that a completed flow's root reply contains the handle, not the body. *(Added artifact delivery guidance to `ROOT_OPERATING_CONTEXT` in `prompt.ts`: "name the deliverable handle and stop. NEVER paste the artifact body into your reply". The Layer-1 test in `App.test.tsx` verifies root's reply contains the handle, not the body.)*
...
- [ ] Layer-4 VHS evidence (same bar as Plan 10/13): drive a delegation to completion → screenshot the bar → open the viewer (markdown body on screen) → open the jump list; file assertions confirm artifacts exist and the body is NOT in scrollback. *(Deferred — requires slow-mode e2e model + tape + assertions, significant additional effort.)*
