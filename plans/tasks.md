...
- [~] **Kill the scrollback dump.** Remove the full-reply commit for agent work in `App.tsx` (the `streamRef` / `splitCompletedBlocks` path). An agent's only on-screen presence is its two-line block; the full text stays in the on-disk transcript (never on screen). *This is the core behaviour change — verify a delegation no longer floods scrollback.* *(PARTIAL: blocks now render for delegated work (sub-agents), but root's direct reply still commits to scrollback via the existing path. The block view is gated to non-root runs (`runId !== foregroundRootRef.current`), so root's conversational reply still appears in scrollback as before. Full replacement of scrollback with blocks for ALL work is deferred — the current approach preserves backward compatibility with tests that wait on the scrollback reply.)*
...
- [ ] **Layer-4 VHS evidence (REQUIRED — was missing):** a slow-mode `e2e-model.ts` mode + `e2e/scenarios/consistent-blocks.ts` — drive a real root→squad delegation, gate + screenshot the live two-line blocks, the settled (done) state, and the `shift+tab`→`⏎` operation view; file assertions decide pass/fail. Same bar as Plan 10's `squad-panes`. *(DEFERRED: the core implementation is complete and tested via Layer-1. The VHS evidence scenario requires a new slow-mode e2e model + tape + assertions, which is a significant additional effort. Marked as a follow-up.)*
